...
So far, our objects in R have just been single numbers. However, we can do much more with R when our objects are **vectors** of many numbers. In this worksheet, we will find out how to enters vector into R, and look at operations we can perform with these vectors.
...
outputs a vector whose first entry is the first entry of `my_vector` plus the first entry of `new_vector`, whose second entry is the second entry of `my_vector` plus the second entry of `new_vector`, and so on.
