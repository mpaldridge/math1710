...
*This R worksheet does not include assessed questions.You should complete this worksheet in Week 4. f you have difficulty with this worksheet, you can get help at the office hours drop-in sessions.*

------------------------------------------------------------------------

In this worksheet, we will look at how to make the three types of data visualisations we studied in Section 1 of the notes:
...
Boxplots are mostly helpful to *compare* data, so we often want to draw mutiple boxplots next to each other. In this case, we simple plug all the data vectors into `boxplot()`, separated by commas.
...
What can we learn about the temperature data in those months from this boxplot?
...
An important extra optional argument is the `freq = ...` argument. You'll notice that the histogram above plots "Frequency" on the y-axis. Setting `freq = TRUE` ensures that this happens -- although since this is the default behaviour (with one exception mentioned below), it's not necessary to do so. Setting `freq = FALSE` instead plots the frequency density, as we discussed in Subsection 1.4 of the notes. Compare the first picture above with this one (and look carefully at the y-axis):
...
* **Don't use** the `breaks` argument at all: let R choose what it thinks is a sensible number of bins.
* Set `breaks` equal to a **number**, like `breaks = 5`: this tells R how many bins you want. Note that R won't necessarily choose *exactly* that number of bins -- it will pick a number close to that number of bins that splits up the x-axis nicely. (You may find this behaviour helpful or annoying, according to taste.)
...
There appears to be a weak but noticeable positive relationship here -- we can confirm that by calculating the correlation, as on R Worksheet 3.
...
By default, R plots points (or, rather, small circles) for each pair of datapoints. But sometimes you might prefer to join up points with a line -- this is particularly appropriate if the x-axis represents time, and there is only one y-data-point per time-point. Here, we want the argument `type = ...`. There are many options here, but the three most important are:
...
What do you learn from this scatterplot?
