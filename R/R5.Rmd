...
* In a scatterplot with `plot()`, the argument **``pch = ...``** sets the character used to plot points. The default os `pch = 1`, which is a circle. Try setting `pch` to other numbers to see what happens.
If you want to draw a plot with one or both axes on a logarithmic scale (or "log scale), try setting `log = ...` to `log = "x"`, `log = "y"`, or `log = "xy"`.
...
**First**, read  the data set at `https://mpaldridge.github.io/math1710/data/R5-xy.csv` into R. Remember that `xy` should be replaced by the last 2 digits of your student ID number. This data set has ten columns, called `alpha`, `beta`, `gamma`, `delta`, `epsilon`, `zeta`, `eta`, `theta`, and `iota`.

> <span style="color: red;">**Note:** The datasets are not currently working correctly (morning of Monday 25 October). You can start planning the appropriate commands to use to answer the questions and save those commands in an R script for later, but you can't run them on the actual data yet. I will try to get this sorted very soon, and will update this message when everything is working properly.</span>
