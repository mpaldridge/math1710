---
title:  "R Worksheet 6: Discrete distributions"
author: "MATH1710 Probability and Statistcs I"
date:   "University of Leeds, 2023--25"
output: html_document
---

*There* are *assessed questions associated with this worksheet, at the bottom of this worksheet. The deadline for submitting your solutions to these questions is **Monday 20 November at 1400**. If you have difficulty with this worksheet, you can get help at the office hours drop-on sessions.*
...
-   **`d`** denotes the **probability mass function** (PMF) $p_X(x) = \mathbb P(X = x)$. (The letter `d` is for "density" -- although, strictly speaking, the word "density" only actually applies to continuous random variables.)
...
or just `dbinom(5, 10, 0.4)`, for short.
...
> **(b)** By using `dbinom()`, the `:` notation, and `sum()`, calculate $\mathbb P(13 \leq X \leq 17)$.
...
Suppose $X \sim \text{Bin}(10, 0.4)$ again. Then the probability $\mathbb P(X \leq 6)$ that $X$ is at most 6 is
...
-   `lower-tail = FALSE` to instead calculate the upper-tail probability $1 - F(x) = \mathbb P(X > x)$. Note that that is *strictly greater* than $x$, *not* greater-than-or-equal-to.

> **Exercise 7.2.** Let $X \sim \text{Bin}(20, 0.6)$ again.\
> **(a)** Calculate $\mathbb P(X \leq 12)$.\
> **(b)** Calculate $\mathbb P(X \leq x)$ for all $x$ between 0 and 20, with all answers rounded to 2 decimal places.\
> **(c)** Calculate $\mathbb P(X \geq 16)$. (Careful: that's a greater-than-*or-equal* sign.)

Third, **`qbinom()`** gives the **quantile function**. That is, for $0 \leq f \leq 1$, the command `qbinom(q, n, p)` gives the value $x$ such that $F(x) = q$, where $F(x) = \mathbb P(X \leq x)$, if there is such an $x$. If $F(x) = q$ does not have an exact solution, then `qbinom(q, n, p)` gives the smallest $x$ such that $F(x) \geq q$. To put it another way, the quantile function is the inverse of the CDF, $F^{-1}(q) = x$. To put it yet another way, `qbinom()` answers the question "How large an $x$ do I need to be at least $100q\%$ sure that $X \leq x$.
...
> **Exercise 7.5.** Explain in mathematics what the following lines of R code have calculated:
...
> **Exercise 7.6.** Let $X \sim \text{Bin}(500, 0.01)$. Calculate exactly:\
> **(a)** $\mathbb P(X = 4)$;\
> **(b)** $\mathbb P(X \geq 7)$.\
...
-   To find the quantile function under our definition, you need `qgeom(q, p) + 1`.
...
*The following five assessed questions should be submitted via [this Microsoft Form](https://forms.office.com/Pages/ResponsePage.aspx?id=qO3qvR3IzkWGPlIypTW3ywARQdZlKXRHsLcXi_ngX8NUOUhHUEVBOTJWRkNOSktPU083UFRLNDRHRC4u). I recommend you do this in Week 7, but the official deadline is **Monday 20 November at 1400**.*
...
> **Assessed Question 1.** *Let $X_1 \sim \text{Bin}(n ,p)$, where $n = 20 + i$ and $p = (1 + i)/20$, where $i$ is the final digit of your Student ID number. What is $\mathbb P(X_1 = 3)$? Round your answer to three significant figures.*

> **Assessed Question 2.** *Let $X_2 \sim \text{Bin}(n,p)$, where $n = 200 + i$ and $p = (i+10)/20$, where $i$ is the final digit of your Student ID number. What is $\mathbb P(X_2 \text{ is even})$? Round your answer to four significant figures.*

> **Assessed Question 3.** *Let $X_3 \sim \text{Bin}(n,p)$, where $n = 200 + i$ and $p = (20 + i)/1000$, where $i$ is the final digit of your Student ID number. Let $Y_3$ be the Poisson approximation to $X_3$. What is $\mathbb P(Y_3 \leq 4)$? Round your answer to four significant figures.*
...
> **Assessed Question 5.** *Let $X_5 \sim \text{Po}(\lambda)$, where $\lambda = (20 + i)/2$ What is $\mathbb P(20 \leq X_5 \leq 30)$? Round your answer to three significant figures.*
