...
*This R worksheet does not include any assessed questions. Answers to the exercises will be made available on Monday 4 October. If you have difficulty with this Worksheet, you can get help at your R troubleshooting drop-in in Week 2*
...
R also allows you to put linebreaks in any command *that is obviously not yet finished*. So if you enter `2 +` and press Enter, R will know that command is unfinished. The next line will begin with a blue `+`, where you can continue the command. Similar things happen if you have opened a pair of brackets without closing them.
...
Names of objects are case-sensitive, so if you want the object `special`, you can't write `SPECIAL` or `Special`. Note that, when doing mathematics, we usually give variables a single letter like $x$ or $y$; but when we're doing programming, we usually find it helpful to use longer descriptive names, like `temperature_data` or `current_total`.
...
This might seem bizarre -- it would be nonsensical to write $x = x + 10$ as mathematical equation! But the key is to remember that `<-` means "assignment"; so this command means "assign the old value of `very_special` plus 10 to the new version of the object `very + special`.
...
**(d)** Add 5 to the value of `ringo`.\
**(e)** Check the new value of `ringo`. (It should be 2021.)
...
You have now finished R Worksheet 1. If you had difficulty with any of the exercises, remember that there are R troubleshooting drop-in sessions in Week 2, where you can ask for help.

You can now close RStudio. When you close RStudio, R might ask you if you want save unsaved work on your R Script (probably you do!). It may also ask you if you want to save your workspace -- that is, save all the objects you've created while working on the worksheet. It's considered best practice to *not* save the workspace -- it will end up getting cluttered up with all the objects you create, but you've already saved the work you need in your R Script.