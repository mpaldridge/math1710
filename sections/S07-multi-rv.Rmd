...
::: {.example}
*Let $X_1, X_2, \dots, X_{10}$ be IID random variables following a Poisson distribution with rate $\lambda = 3$. What is the probability that all the $X_i$ are nonzero?*
...
## Expectation sums and products {#sum-product}
...
Recall that, if $X$ and $Y$ are independent, then $\Cov(X,Y) = 0$. Hence it follows that if $X$ and $Y$ are idnependent, then $\Corr(X,Y) = 0$ also.
...
&=   \frac{1}{n^2} (\Var(X_1) + \Var(X_2) + \cdots + \mathbb EX_n)\\
...
So the long-run proportion of times an event happens converges to its probability. This goes right back to what we said about "frequentist probability" [right at the beginning of Section 2](#what-is-prob): that one way to understand the probability of an event is as the long-run frequency of its occurrence. 
